...
function sendSubscriberSummary($chat_id, $subItem, $bale_log_file)
{
    ...
    foreach ($subItem['InfoItems'] as $info) {
        ...
        if ($info['Code'] == 1){
            $cityName=$info['Value'];
            $summaryText .= "شهر: " . $info['Value'] . "\\n";
        }
        if ($info['Code'] == 10){
            $ownerName=$info['Value'];
            $summaryText .= "مالک: " . $info['Value'] . "\\n";
        }
        if ($info['Code'] == 4){
            $spassword=$info['Value'];
            $summaryText .= "رمز رایانه: " . $info['Value'] . "\\n";
        }
        if ($info['Code'] == 16)
            $paylink = $info['Value'];
    }
    ...
    foreach ($subItem['LastBillItems'] as $lastbill) {
        if ($lastbill['Code'] == 103){
            ...
            $summaryText .= "تاریخ آخرین قرائت: " . $lastbill['Value'] . "\\n";
        }
        if ($lastbill['Code'] == 104){
            $lastmeter= $lastbill['Value'];
            $summaryText .= "عدد کنتور: " . $lastbill['Value'] . "\\n";
        }
    }
    ...
    return ['SubscriberNo'=>$subNo,'cityCode'=>$cityCode,'cityName'=>$cityName,'ownerName'=>$ownerName,'spassword'=>$spassword,
    'paylink'=>$paylink,'lastread'=>$lastread,'lastmeter'=>$lastmeter,'debtAmount'=>$debtAmount,'billId'=>$billId,'paymentId'=>$paymentId];
}

function sendmainmenu($chat_id, $accessToken, $abfacs_log_file, $sessions, $user_id, $bale_log_file, $session_file)
{
    if (!$accessToken) {
    ...
    if (!$info || (isset($info['Execute']) && $info['Execute'] === false) || empty($info['Items'])) {
        ...
        foreach ($info['Items'] as $sub) {
            ...
            //$sessions[$user_id]['subscribers']=array_merge($sessions[$user_id]['subscribers'],sendSubscriberSummary($chat_id, $sub, $bale_log_file));
        }
        $row = '"reply_markup":{"inline_keyboard":[[
                            {"text" : "اضافه کردن اشتراک جدید", "callback_data" : "add_sub_' . $chat_id . '"}]]}';
        sendMessageToBale($chat_id, "سایر", $bale_log_file, $row);
        saveSessions($user_id,$sessions[$user_id]);
    }
}
...
//ثبت رقم کنتور
function declareMeter1522($subscriberNo, $meterNo, $phone, $logFile, $serviceUrl = ABFACS_ASMX, $companyCode = COMPANYCODE)
{
    try {
            ...
            if (isset($msg['text'])) {
                ...
                //echo "check waiting_for_billid passed\n";
                    ...
                    if (!$info || (isset($info['Execute']) && $info['Execute'] === false) || empty($info['Items'])) {
                        ...
                        logData($abfacs_log_file, 'REGISTER_ATTEMPT', array(
                            'user' => $user_id,
                            'subscriberNo' => $billId,
                            'cityCode' => $cityCode,
                            'phone' => $phone
                ...
                if (isset($sessions[$user_id]['step']) && strpos($sessions[$user_id]['step'], 'waiting_for_meter_') === 0) {
                    ...
                    $info = declareMeter1522($scode, $counterno, $phone, $abfacs_log_file);
                    if (isset($info) && isset($info['succcess'])) {
                        $sendtxt = $info['statusMessage'] . "\\n";
                        if ($info['succcess']) {
                            $sendtxt .= "مبلغ محاسبه شده:" . $info["billAmount"] . "\\n";
                            $sendtxt .= "شناسه قبض:" . $info["billId"] . "\\n";
                            $sendtxt .= "شناسه پزداخت:" . $info["paymentId"] . "\\n";
                        }
